...
public class StepAppOpenHelper extends SQLiteOpenHelper {
    ...
    public static Map<String, Integer> loadStepsByDay(Context context) {
        ...
        Cursor cursor = database.rawQuery("SELECT day, COUNT(*) AS total_steps FROM " +
                TABLE_NAME + " WHERE day >= date('now', '-5 days') GROUP BY day ORDER BY day ASC", null);

        cursor.moveToFirst();
        for (int index=0; index < cursor.getCount(); index++){
                String day = cursor.getString(0);
                Integer totalSteps = cursor.getInt(1);
                map.put(day, totalSteps);
